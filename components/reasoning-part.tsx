...
const MarkdownRenderer = React.memo(({ content }: { content: string }) => {
  ...
  const renderer = {
    ...
    hr() {
      return (
        <hr key={Math.random()} className="my-3 border-t border-border/80" />
      );
    },
    table(children: ReactNode[]) {
      return (
        <div key={Math.random()} className="overflow-x-auto mb-2">
          <table className="min-w-full border border-border/60 rounded text-xs">
            {children}
          </table>
        </div>
      );
    ...
    tableCell(children: ReactNode[], flags: TableFlags) {
      ...
      const childrenWithKeys = Array.isArray(children)
        ? children.map((child, index) => (
            <React.Fragment key={`cell-child-${index}`}>{child}</React.Fragment>
          ))
        : children;
      ...
      return flags.header ? (
        ...
        <td
          className={`px-1.5 py-1 text-muted-foreground border border-border/60 ${align}`}
        >
          {childrenWithKeys}
        </td>
      );
    ...
    return (
      <div className="my-2" key={sectionKey}>
        <div
          className={cn(
            "bg-accent",
            "border border-border/80 rounded-lg overflow-hidden",
          )}
        >
                      ...
                      <div className="text-muted-foreground prose prose-sm max-w-none">
                        {hasNonEmptyReasoning ? (
                          !isThinking ? (
                            ...
                            <p className="text-muted-foreground whitespace-pre-wrap wrap-break-words">
                              {part.text}
                            </p>
                          )
                        ) : (

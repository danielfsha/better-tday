...
const EnhancedErrorDisplay: React.FC<EnhancedErrorDisplayProps> = ({
  ...
  return (
    <div className="mt-3">
      <div
        className={`rounded-lg border ${colors.border} bg-background dark:bg-background overflow-hidden`}
      >
        <div
  ...
  const handleFilesUpload = useCallback(
    async (files: File[], onFinish: () => void) => {
      ...
      if (unsupportedFiles.length > 0) {
        sileo.error({
          title: `Some files are not supported: ${unsupportedFiles.map((f) => f.name).join(", ")}`,
        });
      }

      if (oversizedFiles.length > 0) {
        sileo.error({
          title: `Some files exceed the size limit: ${oversizedFiles.map((f) => f.name).join(", ")}`,
        });
      }
      ...
      if (totalAttachments > MAX_EDITOR_FILES) {
        sileo.error({
          title: `You can only attach up to ${MAX_EDITOR_FILES} files.`,
        });
        onFinish();
        return;
      }
      ...
      if (imageFiles.length > 0) {
        try {
          ...
          sileo.error({
            title: "Unable to verify image safety. Please try again.",
          });
          onFinish();
          return;
        }
      ...
      try {
        ...
        if (uploadedAttachments.length > 0) {
          ...
          setMessages((currentMessages) => {
            ...
            const currentParts = Array.isArray(currentMessage.parts)
        ...
        sileo.error({
          title: "Failed to upload one or more files. Please try again.",
        });
      } finally {
          ...
          status !== "streaming" && (
            <div className="w-full max-w-xl sm:max-w-2xl mt-4">
              <div className="flex items-center gap-1.5 mb-2 pr-3">
                ...
                />
                <h2 className="font-medium texl-lg text-foreground dark:text-foreground">
                  Follow-up
                </h2>
              </div>
              <div className="flex flex-col border-t border-border dark:border-border">
                {suggestedQuestions.map((question, i) => (
                  ...
                  >
                    <CornerDownRight
                      size={16}
                      className="text-primary shrink-0 pr-1"
                    />
...
export const EditableAttachmentsBadge = ({
  ...
  const getEditableDocumentType = (
    attachment: Attachment,
  ): "csv" | "docx" | "xlsx" | "pdf" | "image" | null => {
            ...
            <div className="flex-1 p-1 overflow-auto flex items-center justify-center">
              <div className="relative flex items-center justify-center w-full h-full">
                {isPdf(fileAttachments[selectedIndex]) ? (
                  ...
                  (() => {
                    const editDocType = getEditableDocumentType(
                      fileAttachments[selectedIndex],
                    );
                    if (
                      editDocType === "csv" ||
                      editDocType === "xlsx" ||
              ...
              <div className="border-t border-neutral-200 dark:border-neutral-800 p-2">
                <div className="flex items-center justify-center gap-2 overflow-x-auto py-1 max-w-full">
                  {fileAttachments.map((attachment, idx) => {
                    const thumbEditDocType =
                      getEditableDocumentType(attachment);
                    return (
                      <button
                        key={idx}

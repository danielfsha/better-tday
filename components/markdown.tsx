...
const NON_DOWNLOADABLE_EXTENSIONS = new Set([
  "html",
  "htm",
  "php",
  "asp",
...
function extractFilenameFromHref(href: string, fallbackText?: string): string {
  ...
  const decodedPathSegment = rawPathSegment
    ? decodeURIComponent(rawPathSegment)
    : "";

  if (decodedPathSegment) return decodedPathSegment;

  const candidateFromText = fallbackText?.trim();
  if (candidateFromText && candidateFromText.includes("."))
    return candidateFromText;

  return "download";
}

function getFilePreviewDefinition(
  href: string,
  fallbackText?: string,
): FilePreviewDefinition | null {
  const url = parseUrlLike(href);
  ...
  const hasKnownDownloadExtension = Boolean(
    ext && FILE_TYPE_MAP[ext] && !NON_DOWNLOADABLE_EXTENSIONS.has(ext),
  );
  ...
  const isLikelyFile =
    ...
    hasKnownDownloadExtension ||
    Boolean(hasDownloadHint);
  ...
  return {
    filename,
    title:
      fallbackText?.trim() && fallbackText.trim() !== href
        ? fallbackText.trim()
        : filename,
...
const CodeBlockInner: React.FC<CodeBlockInnerProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      stored &&
      children.length > stored.contentLength &&
      !stored.userCollapsed
    ) {
  ...
  const syntaxTheme = useMemo(() => {
    ...
    const isLightTheme =
      resolvedTheme === "light" ||
      resolvedTheme === "claudelight" ||
      resolvedTheme === "neutrallight";
    ...
    setIsWrapped((prev) => {
      ...
      sileo.success({
        ...
        description: newState
          ...
          : "Long lines will scroll horizontally",
          ...
          {!isCollapsed && (
            <button
              ...
              className={cn(
                ...
                isWrapped
                  ? "text-primary"
                  : "text-muted-foreground/50 hover:text-foreground active:text-foreground",
              )}
...
const SyncCodeBlock: React.FC<CodeBlockProps> = (props) => (
  <CodeBlockInner {...props} allowPlainTextFallback={false} />
);
    ...
    try {
      ...
      for (const { regex, prefix, target } of richTagPatterns) {
        ...
        while ((richTagMatch = regex.exec(modifiedContent)) !== null) {
          ...
          richTagProcessed +=
            modifiedContent.slice(lastRichTagIndex, richTagMatch.index) + id;
        ...
        for (const pattern of patterns) {
          ...
          while ((match = regex.exec(modifiedContent)) !== null) {
            ...
            if (!isBlock) {
              ...
              const containsUrlOrMarkdown = /https?:\/\/|\]\(|:\/\//.test(
                inner,
              );
              ...
              const hasEdgeWhitespace =
                isDollarDelimited && (/^\s/.test(inner) || /\s$/.test(inner));
              const isSingleLetterVar =
                hasEdgeWhitespace && /^\s*[a-zA-Z]\s*$/.test(inner);

              if (
                currencyLike ||
                containsUrlOrMarkdown ||
                isTooLong ||
                (hasEdgeWhitespace && !isSingleLetterVar)
              ) {
      ...
      for (let i = 0; i < citationMatches.length; i++) {
        ...
        if (!text) {
          ...
          citationProcessed += modifiedContent.slice(
            lastCitationIndex,
            matchEnd,
          );
      ...
      function extractLinksWithLexer(text: string): Array<{
        raw: string;
        href: string;
        text: string;
        index: number;
        ...
        try {
          ...
          for (const linkToken of linkTokens) {
            ...
            const escapedHref = linkToken.href.replace(
              /[.*+?^${}()|[\]\\]/g,
              "\\$&",
            );
            ...
            if (match && match[0] === linkToken.raw) {
              ...
              try {
                ...
                const validateLink = validateTokens.find(
                  (t) => t.type === "link",
                );
                if (
                  validateLink &&
            ...
            if (rawIndex !== -1) {
              ...
              if (hrefIndex !== -1) {
                ...
                const searchEnd = Math.min(
                  text.length,
                  hrefIndex + linkToken.href.length + 10,
                ); // Small lookahead for closing paren
                ...
                const bracketPos = searchWindow.lastIndexOf(
                  "[",
                  hrefIndex - searchStart,
                );
                let linkFound = false;

                if (bracketPos !== -1) {
                  // Try to match the full link pattern from this bracket position
                  const patternFromBracket = new RegExp(
                    `\\[([^\\]]+)\\]\\(${escapedHref}\\)`,
                  );
                  const matchFromBracket = searchWindow
                    .slice(bracketPos)
                    .match(patternFromBracket);
                  if (matchFromBracket) {
                    ...
                    if (windowMatch) {
                      const absoluteIndex =
                        searchStart + searchWindow.indexOf(windowMatch[0]);
                      links.push({
                        raw: windowMatch[0],
                        href: linkToken.href,
                        text: windowMatch[1],
      ...
      if (allLinks.length >= 2) {
        ...
        if (groups.length > 0) {
          ...
          for (let g = groups.length - 1; g >= 0; g--) {
            ...
            groupProcessed =
              groupProcessed.slice(0, group.startIndex) +
              groupId +
              groupProcessed.slice(group.endIndex);
          }
      ...
      if (typeof tableBlocks !== "undefined" && tableBlocks.length > 0) {
        for (let t = 0; t < tableBlocks.length; t++) {
          ...
          for (let g = groups.length - 1; g >= 0; g--) {
            ...
            // Processing right-to-left means replacements don't affect earlier indices
            newRow =
              newRow.slice(0, group.startIndex) +
              groupId +
              newRow.slice(group.endIndex);
          }
      ...
      monetaryBlocks.forEach(({ id, content }) => {
        ...
        for (let i = 0; i < latexBlocks.length; i++) {
          latexBlocks[i].content = latexBlocks[i].content.replace(
            regex,
            content,
          );
        }
...
const LinkPreviewContent = ({
  ...
  const displayTitle =
    metadata?.title && !isAccessDenied ? metadata.title : title;
...
const AppLinkPreview = React.memo(
  ({ href, title }: { href: string; title?: string }) => {
    const previewHref = useMemo(() => getAppPreviewHref(href), [href]);
    const screenshotSrc = useMemo(
      () => getAppPreviewScreenshotSrc(href),
      [href],
    );
    const previewDescription = useMemo(
      () => getAppPreviewDescription(href),
      [href],
    );
    const previewTitle =
      title?.trim() && title.trim() !== href
        ? title.trim()
        : "Open deployed app";
          ...
          >
            {hasScreenshotError || !screenshotSrc ? (
              <span className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-[radial-gradient(circle_at_top,#ffffff14,transparent_55%),linear-gradient(180deg,#0f172a,#111827)] text-white/85">
                ...
                <span className="px-4 text-center text-[11px] leading-relaxed text-white/50">
                  Live app
                </span>
              </span>
            ) : (
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0 flex items-baseline gap-2">
          <h5 className="text-[11px] font-medium text-foreground truncate m-0 flex-1">
            {text}
          </h5>
          <span className="text-[9px] text-muted-foreground/60 m-0 shrink-0">
            {platformLabel}
          </span>
        </div>
      </Link>
...
const CitationGroup = React.memo(
  ({ urls, texts, elementKey }: CitationGroupProps) => {
    const isMobile = useIsMobile();
    const [isOpen, setIsOpen] = useState(false);
...
const FileLinkPreview = React.memo(
  ({ href, title }: { href: string; title?: string }) => {
    const [isDownloading, setIsDownloading] = useState(false);
    const preview = useMemo(
      () => getFilePreviewDefinition(href, title),
      [href, title],
    );
...
const ImageWithPreview = React.memo(
  ({ src, alt, title }: { src: string; alt?: string; title?: string }) => {
    ...
    return (
      <span
        className="block my-2 relative group"
      ...
      >
        ...
        {alt && alt !== "" && alt !== "img" && (
          <span className="block text-xs text-muted-foreground my-1 text-center">
            {alt}
          </span>
        )}
      </span>
  ...
  ({ content, isUserMessage = false }) => {
    ...
    const renderCitation = useCallback(
      (index: number, citationText: string, href: string, key: string) => {
        ...
        return (
          <>{renderHoverCard(href, displayDomain, true, citationText, key)}</>
        );
      },
      [renderHoverCard],
    );
    ...
    const renderer: Partial<ReactRenderer> = useMemo(
      () => ({
        text(text: string) {
          ...
          const hasLatex =
            text.includes("XLATEXBLOCKX") || text.includes("XLATEXINLINEX");
          ...
          allMatches.forEach(({ match, type }) => {
            ...
            if (type === "citation-group") {
              const citationGroup = citationGroups.find(
                (group) => group.id === fullMatch,
              );
              if (citationGroup) {
              ...
              const appPreview = appPreviewBlocks.find(
              ...
              if (appPreview) {
                ...
                components.push(
                  <AppLinkPreview
                    key={key}
                    href={appPreview.href}
                    title={appPreview.title}
                  />,
                );
              }
            } else if (type === "download") {
              const download = downloadBlocks.find(
                (block) => block.id === fullMatch,
              );
              if (download) {
        ...
        link(href, text) {
          ...
          const filePreview = getFilePreviewDefinition(
            href,
        ...
        heading(children, level) {
          ...
          return (
            <HeadingTag
              key={key}
              className={`${sizeClasses} text-foreground tracking-tight scroll-mt-20`}
            >
              {children}
            </HeadingTag>
          );
        ...
        listItem(children) {
          ...
          return (
            <li
              key={key}
              className="pl-2 text-[15px] leading-relaxed text-foreground/90"
            >

    ...
    <div className="p-4 space-y-5 text-sm">
      ...
      <Section title="Position">
        ...
        <Row>
          <Field label="W">
            <NumInput
              value={Math.round((o.width ?? 0) * (o.scaleX ?? 1))}
              onChange={(v) => onChange({ scaleX: v / (o.width ?? 1) })}
            />
          </Field>
          <Field label="H">
            <NumInput
              value={Math.round((o.height ?? 0) * (o.scaleY ?? 1))}
              onChange={(v) => onChange({ scaleY: v / (o.height ?? 1) })}
            />
          </Field>
        </Row>
        <Row>
          ...
          <Field label="Opa">
            <NumInput
              ...
              onChange={(v) =>
                onChange({ opacity: Math.max(0, Math.min(1, v / 100)) })
              }
            />
          </Field>
        </Row>
        ...
        <>
          ...
          <Section title="Auto Layout">
            <div className="flex gap-1 rounded-md bg-secondary p-0.5">
              <SegBtn
                active={!layout || layout.mode === "free"}
                onClick={() => onLayout({ mode: "free" })}
              >
                Free
              </SegBtn>
              <SegBtn
                active={!!layout && layout.mode === "flex"}
                onClick={() => onLayout({ mode: "flex" })}
              >
                Flex
              </SegBtn>
            </div>
            {layout?.mode === "flex" && (
              <>
                <div className="flex gap-1 rounded-md bg-secondary p-0.5">
                  <SegBtn
                    active={layout.direction === "row"}
                    onClick={() => onLayout({ direction: "row" })}
                  >
                    <MoveHorizontal size={12} /> Row
                  </SegBtn>
                  <SegBtn
                    active={layout.direction === "column"}
                    onClick={() => onLayout({ direction: "column" })}
                  >
                ...
                <div className="flex gap-1 rounded-md bg-secondary p-0.5">
                  <SegBtn
                    active={(layout.align ?? "start") === "start"}
                    onClick={() => onLayout({ align: "start" })}
                  >
                    <AlignStartVertical size={12} />
                  </SegBtn>
                  <SegBtn
                    active={layout.align === "center"}
                    onClick={() => onLayout({ align: "center" })}
                  >
                    <AlignCenterVertical size={12} />
                  </SegBtn>
                  <SegBtn
                    active={layout.align === "end"}
                    onClick={() => onLayout({ align: "end" })}
                  >
      ...
      <Section title="Fill">
        <ColorRow value={fill} onChange={(v) => onChange({ fill: v })} />
      </Section>
      ...
      <Section title="Stroke">
        ...
        <Row>
          ...
          {"rx" in o && (
            <Field label="Rad">
              <NumInput
                value={(o as fabric.Rect).rx ?? 0}
                onChange={(v) => onChange({ rx: v, ry: v })}
              />
            </Field>
          )}
        </Row>

...
export function MeasurementBadge({ getCanvas, vp, enabled }: Props) {
  ...
  useEffect(() => {
    const c = getCanvas(); if (!c || !enabled) return;
    ...
    c.on("after:render", h);
    return () => {
      c.off("selection:created", h); c.off("selection:updated", h);
      c.off("selection:cleared", h); c.off("object:moving", h);
      c.off("object:modified", h); c.off("after:render", h);
    };
  }, [getCanvas, enabled]);
  void tick;
  ...
  return (
    <svg className="pointer-events-none absolute inset-0 h-full w-full">
      <line x1={ax} y1={ay} x2={bx} y2={by} stroke="#22d3ee" strokeWidth={1} strokeDasharray="4 4" />
      <foreignObject x={mx - 40} y={my - 12} width={80} height={24}>
        <div className="rounded bg-[#22d3ee] px-1.5 py-0.5 text-center text-[10px] font-semibold text-black shadow">
          {dist}px <span className="opacity-60">· {dx},{dy}</span>
        </div>
      </foreignObject>
    </svg>
  );

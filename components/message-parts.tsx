...
} from "@/components/tool-invocation-list-view";
...
const InteractiveChart = dynamic(
  () => import("@/components/interactive-charts"),
  { ssr: false },
);
const MapComponent = dynamic(
  () =>
    import("@/components/map-components").then((m) => ({
      default: m.MapComponent,
    })),
  {
    ssr: false,
  },
);
const NearbySearchMapView = dynamic(
  () => import("@/components/nearby-search-map-view"),
  { ssr: false },
);
const InteractiveStockChart = dynamic(
  () => import("@/components/interactive-stock-chart"),
  { ssr: false },
);
const CryptoChart = dynamic(
  () =>
    import("@/components/crypto-charts").then((m) => ({
      default: m.CryptoChart,
    })),
  {
    ssr: false,
  },
);
const CryptoTickers = dynamic(
  () =>
    import("@/components/crypto-charts").then((m) => ({
      default: m.CryptoTickers,
    })),
  {
    ssr: false,
  },
...
function McpAppOutputBlock({
  ...
  const [resourceMeta, setResourceMeta] = useState<McpUiResourceMeta | null>(
    null,
  );
  ...
  const sandboxUrl = useMemo(
    () =>
      app.kind === "ui-resource" && app.resourceUri
        ? getSandboxUrl(app.resourceUri)
        : null,
  ...
  const proxyBridgeRequest = useCallback(
    async (method: string, params: Record<string, unknown> = {}) => {
      if (!app.serverId)
        throw new Error("Missing MCP server metadata for app bridge");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const meta = (payload?.resourceMeta ??
          null) as McpUiResourceMeta | null;
  ...
  useEffect(() => {
    ...
    bridge.oncalltool = async (params) =>
      proxyBridgeRequest("tools/call", params as Record<string, unknown>);
    bridge.onlistresources = async (params) =>
      proxyBridgeRequest("resources/list", params as Record<string, unknown>);
    bridge.onreadresource = async (params) =>
      proxyBridgeRequest("resources/read", params as Record<string, unknown>);
    ...
    bridge.onlistprompts = async (params) =>
      proxyBridgeRequest("prompts/list", params as Record<string, unknown>);
        ...
        try {
          ...
          if (!cancelled) {
            ...
            setLoadError(
              error instanceof Error
                ? error.message
                : "Failed to initialize MCP App bridge",
            );
          }
        }
      })();
    ...
    <div className="px-3.5 py-2.5">
      <div className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2.5 text-[11px] text-muted-foreground space-y-1.5">
        ...
        {loadError ? (
          ...
          <p>
            Server returned a UI resource reference. Missing server metadata
            prevented loading.
          </p>
        )}
        {app.serverName && (
        ...
        {app.resourceUri && (
          <code className="block text-[10px] text-muted-foreground/80 break-all">
            {app.resourceUri}
          </code>
        )}
      </div>
...
const McpApprovalCard = ({
  ...
  const previewEntries = approval.preview
    ? Object.entries(approval.preview)
    : [];
  ...
  const handleConfirm = () => {
    ...
    sendMessage({
      text: `Approved. Please proceed — approvalToken: ${approval.approvalToken}`,
    });
  };
          ...
          <div className="divide-y divide-border/30">
            {previewEntries.map(([key, val]) => (
              <div key={key} className="px-3 py-1.5 flex gap-3 items-start">
                <span className="text-[11px] text-muted-foreground/60 capitalize w-20 shrink-0 pt-px">
                  {key}
                </span>
      ...
      if ("state" in part && part.state) {
        switch (part.type) {
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="text-[11px] px-2 py-0.5 rounded-full bg-[hsl(var(--secondary))] text-[hsl(var(--secondary-foreground))]">
                              {part.output.search_type === "forward"
                                ? "Address Search"
                                : "Coordinate Search"}
                            </span>
                          </div>
              ...
              case "input-available":
                return (
                  <StockChartLoader
                    key={`${messageIndex}-${partIndex}-tool`}
                    title={
                      part.input?.title || "Preparing financial analysis..."
                    }
                    input={part.input}
                  />
                );
                  ...
                  <InteractiveStockChart
                    ...
                    sec_filings={
                      part.output.sec_filings?.map((filing) => ({
                        ...
                        requestedCompany:
                          "requestedCompany" in filing
                            ? String(filing.requestedCompany)
                            : "N/A",
                        requestedFilingType:
          ...
          case "tool-xai_x_search": {
            const mergedXaiToolParts = parts.filter(
              isXaiMultiAgentToolPart,
            ) as XaiMultiAgentToolPart[];
            const mergedFirstXaiToolIndex = parts.findIndex(
              isXaiMultiAgentToolPart,
            );
            ...
            const labels = mergedXaiToolParts.map(
              (toolPart: XaiMultiAgentToolPart) =>
            ...
            const counts = labels.reduce<Record<string, number>>(
            ...
            const summary = Object.entries(counts)
              .map(([label, count]) => {
                const pluralLabel =
                  label === "Web Search" ? "Web Searches" : label;
          ...
          case "tool-x_search":
            switch (part.state) {
              ...
              case "output-available":
                ...
                const xSearchInput = {
                  ...rawXSearchInput,
                  includeXHandles: rawXSearchInput.includeXHandles?.filter(
                    (h): h is string => h !== undefined,
                  ),
                  excludeXHandles: rawXSearchInput.excludeXHandles?.filter(
                    (h): h is string => h !== undefined,
                  ),
                };
                              ...
                              xSearchOutput.searches?.map((search: any) => ({
                                ...
                                sources:
                                  search.sources?.filter(
                                    (s: any): s is NonNullable<typeof s> =>
                                      s !== null,
                                  ) || [],
          ...
          case "tool-box_exec": {
            ...
            const execAnns = buildAnnotations.filter(
              (a) => a.data.kind === "exec",
            );
          ...
          case "tool-box_write": {
            ...
            const writeAnns = buildAnnotations.filter(
              (a) => a.data.kind === "write",
            );
          ...
          case "tool-box_read": {
            ...
            const readAnns = buildAnnotations.filter(
              (a) => a.data.kind === "read",
            );
          ...
          case "tool-box_list_files": {
            ...
            const listAnns = buildAnnotations.filter(
              (a) => a.data.kind === "list",
            );
          ...
          case "tool-box_download": {
            ...
            const dlAnns = buildAnnotations.filter(
              (a) => a.data.kind === "download",
            );
          ...
          case "tool-box_agent": {
            ...
            const agentAnns = buildAnnotations.filter(
              (a) => a.data.kind === "agent",
            );
            return (
              <BoxAgentResult
          ...
          case "tool-box_code": {
            ...
            const codeAnns = buildAnnotations.filter(
              (a) => a.data.kind === "code",
            );

...
function advancedSearch(chat: Chat, query: string, mode: SearchMode): boolean {
  ...
  if (query.startsWith("today:")) {
    return (
      isToday(getChatActivityDate(chat)) &&
  ...
  if (query.startsWith("week:")) {
    return (
      isThisWeek(getChatActivityDate(chat)) &&
  ...
  if (query.startsWith("month:")) {
    return (
      isThisMonth(getChatActivityDate(chat)) &&
      fuzzySearch(query.slice(6), chat.title)
    );
  }
  ...
  if (query.startsWith("date:")) {
    ...
    return fuzzySearch(
      dateQuery,
      getChatActivityDate(chat).toLocaleDateString(),
    );
  }
...
export function ChatHistoryDialog({
  ...
  const { prefetchOnHover, prefetchOnFocus, prefetchChatRoute } =
    useChatPrefetch();
  ...
  const {
    ...
    queryFn: async ({ pageParam }) => {
      ...
      if (pageParam) {
        ...
        return await loadMoreChats(
          user.id,
          pageParam.id,
          30,
          pageParam.cursorDate,
          pageParam.cursorIsPinned,
        );
      } else {
    ...
    getNextPageParam: (lastPage) => {
      ...
      return {
        id: lastChat.id,
        cursorDate: new Date(
          lastChat.updatedAt ?? lastChat.createdAt,
        ).toISOString(),
    ...
    initialPageParam: undefined as
      | { id: string; cursorDate: string; cursorIsPinned: boolean }
      | undefined,
  ...
  const filteredChats = useMemo(() => {
    const matchingChats = allChats.filter((chat) =>
      advancedSearch(chat, searchQuery, searchMode),
    );
    return matchingChats.sort(
      (a, b) =>
        getChatActivityDate(b).getTime() - getChatActivityDate(a).getTime(),
    );
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      const tasks = Object.fromEntries(
        ids.map((id) => [`chat:${id}`, async () => deleteChat(id)]),
      );
      const settled = await allSettled(tasks, getBetterAllOptions());
      const anyRejected = Object.values(settled).some(
        (r) => r.status === "rejected",
      );
      if (anyRejected) {
  ...
  const pinMutation = useMutation({
    ...
    onSuccess: (updatedChat, { id, isPinned }) => {
      if (updatedChat) {
        ...
        sileo.error({
          title: "Failed to update pinned state. Please try again.",
        });
      }
    },
    onError: (error) => {
  ...
  const allFilteredSelected = useMemo(() => {
    return (
      ...
      filteredChats.every((chat) => selectedChatIds.has(chat.id))
    );
    ...
    return (
      <CommandItem
        ...
        className={cn(
          ...
          isEditing &&
            "bg-muted/30 dark:bg-muted/20 border border-muted-foreground/20 shadow-sm",
          isSelected &&
            "bg-accent border border-accent-foreground/10 shadow-sm ring-1 ring-accent-foreground/5",
          !isDeleting &&
            !isEditing &&
            !isSelected &&
              ...
              {bulkSelectMode ? (
                <>
                  <Button
                    ...
                    onClick={
                      allFilteredSelected ? deselectAllChats : selectAllChats
                    }
                  >
                    {allFilteredSelected ? (
                      <>
                ...
                <div className="flex items-center w-full gap-2">
                  ...
                  <div className="ml-auto flex items-center gap-2 text-xs text-muted-foreground">
                    <div className="flex items-center gap-1">
                      <Kbd className="rounded font-mono">
                        {isMac ? "⌘" : "Ctrl"}
                      </Kbd>
    ...
    <Tooltip>
      <TooltipTrigger>
        <Button
          ...
          aria-label="Chat History"
        >
          <ListMagnifyingGlassIcon className="size-6" />

...
import {
  getMcpCatalogIcon,
  MCP_COMPONENT_ICON_URLS,
} from "@/lib/mcp/catalog-icons";
...
const ModelSwitcher: React.FC<ModelSwitcherProps> = React.memo(
  ({
    ...
    const isMaxUser = useMemo(
      () => Boolean(user?.isMaxUser),
      [user?.isMaxUser],
    );
    ...
    const availableModels = useMemo(
      () => getFilteredModels(countryCode || undefined),
      [countryCode],
    );

    const [showUpgradeDialogState, setShowUpgradeDialogState] = useState(false);
    const [showSignInDialog, setShowSignInDialog] = useState(false);
    const [selectedProModelValue, setSelectedProModelValue] = useState<
      string | null
    ...
    const selectedRequiresMax = useMemo(
      () =>
        selectedProModel
          ? requiresMaxSubscription(selectedProModel.value)
          : false,
      [selectedProModel],
    );
    const setSelectedProModel = useCallback(
    ...
    const [preferredModels, setPreferredModels] = useSyncedPreferences<
      string[]
    ...
    const filteredModels = useMemo(() => {
      ...
      if (
        ...
        const fallback =
          filtered.find((m) => m.value === "scira-code") || filtered[0];
        if (fallback) {
    ...
    const [modelOrderMap] = useLocalStorage<Record<string, string[]>>(
      "scira-model-order",
      {},
    );
      ...
      const normalizedByCategory = normalizedOrder
        ...
        .map((category) => {
          ...
          const positionById = new Map(
            order.map((id, idx) => [id, idx] as const),
          );
    ...
    useEffect(() => {
      ...
      const isCurrentModelRestricted = isModelRestrictedInRegion(
        selectedModel,
      ...
      if (
        ...
        console.log(
          `Auto-switching from max model '${selectedModel}' to 'scira-default' - user lost max access`,
        );
        setSelectedModel("scira-default");
        return;
      }
      ...
      if (
        ...
        console.log(
          `Auto-switching from pro model '${selectedModel}' to 'scira-default' - user lost pro access`,
        );
        setSelectedModel("scira-default");
      }
    }, [
    ...
    const activeProviders = useMemo(() => {
      ...
      return PROVIDER_ORDER.filter(
        (p) => p === "all" || providerSet.has(p as ModelProvider),
      );
    }, [availableModels]);
    ...
    useEffect(() => {
      ...
      const items =
        modelListRef.current.querySelectorAll<HTMLElement>(
          "[data-model-index]",
        );
      ...
      (e: React.KeyboardEvent) => {
        ...
        if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
          ...
          if (providerSidebarRef.current) {
            const buttons =
              providerSidebarRef.current.querySelectorAll<HTMLElement>(
                "button",
              );
                ...
                <TooltipTrigger>
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      if (isFav) {
                        setPreferredModels(
                          preferredModels.filter((id) => id !== model.value),
                        );
                      } else {
                  ...
                  >
                    <Star
                      className={cn(
                        "size-3",
                        preferredModels.includes(model.value) && "fill-current",
                      )}
                    />
                  </button>
                </TooltipTrigger>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  aria-activedescendant={
                    focusedIndex >= 0
                      ? `model-option-${displayModels[focusedIndex]?.value}`
                      : undefined
                  }
                  className={cn(
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground/60 mt-0.5">
                      Starting at ${PRICING.PRO_MONTHLY}/mo
                    </p>
                  </div>
                  <Button
                    type="button"
                    size="sm"
                    className="h-6 text-[10px] px-2.5 rounded-md shrink-0"
                  >
                    Upgrade
                  </Button>
                </div>
              ...
              <div className="items-center gap-1.5 sm:flex hidden">
                {(() => {
                  ...
                  const routedProvider =
                    routedConfig?.provider ||
                    getModelProvider(
                      autoRoutedModel.model,
                      routedConfig?.label || "",
                    );
                  return (
                    <ProviderIcon
                ...
                <span className="text-[13px] font-medium">
                  {getModelConfig(autoRoutedModel.model)?.label ||
                    autoRoutedModel.model}
                </span>
              ...
              <div className="relative z-10">
                {selectedProModel?.label ? (
                  ...
                  <div className="flex items-center justify-center gap-2">
                    <p className="text-3xl font-semibold tracking-tight font-be-vietnam-pro">
                      scira
                    </p>
                    <ProBadge />
                  </div>
                  ...
                  {selectedRequiresMax ? (
                    ...
                    <>
                      <span className="text-sm text-muted-foreground line-through">
                        ${pricing.usd.originalPrice}
                      </span>
                      <span className="text-2xl font-bold">
                        ${pricing.usd.finalPrice.toFixed(2)}
                      </span>
                    </>
                  ) : (
                    <span className="text-2xl font-bold">
                      ${pricing.usd.finalPrice}
                    </span>
                  )}
            ...
            <div className="px-6 pb-6 space-y-4">
              <div className="rounded-xl border border-border/60 overflow-hidden grid grid-cols-2">
                {(selectedRequiresMax
                  ? [
                      {
                        title: "All paid features",
                        desc: "Unlimited searches & more",
                      },
                      {
                        title: "Claude 4.6 Opus",
                        desc: "Most advanced Anthropic LLM",
                      },
                      {
                        title: "Claude 4.6 Opus Thinking",
                        desc: "With extended reasoning",
                      },
                      {
                        title: "Claude 4.5 Opus",
                        desc: "Previous advanced LLM",
                      },
                      {
                        title: "Claude 4.6 Sonnet",
                        desc: "Latest Sonnet model",
                      },
                      { title: "Claude 4.5 Haiku", desc: "Fast and efficient" },
                      {
                        title: "1M context window",
                        desc: "For Anthropic models",
                      },
                      { title: "Canvas support", desc: "Visualization mode" },
                    ]
                  : [
                      {
                        title: "All standard AI models",
                        desc: "GPT-5.2, Gemini 3.1, Grok 4.1",
                      },
                      { title: "Unlimited searches", desc: "No daily limits" },
                      {
                        title: "Extreme research",
                        desc: "Multi-step deep analysis",
                      },
              ...
              {selectedRequiresMax && isProUser && (
                <p className="text-[10px] text-center text-muted-foreground/60 leading-relaxed">
                  Your existing paid subscription will be automatically
                  cancelled once Max is activated.
                </p>
              )}
...
const getMaxSizeForFile = (file: File): number =>
  isImageFile(file) ? MAX_IMAGE_SIZE : MAX_DOCUMENT_SIZE;
...
const ConnectorSelector: React.FC<ConnectorSelectorProps> = React.memo(
  ({ selectedConnectors, onConnectorToggle, user, isProUser }) => {
    ...
    const connectorItems = availableConnectors.map(([provider, config]) => {
      ...
      const isChecked = selectedConnectors.includes(
...
const McpServerSelector: React.FC<McpServerSelectorProps> = React.memo(
  ({ user, isProUser }) => {
    const [open, setOpen] = useState(false);
    ...
    const isReady = (s: { authType: string; isOAuthConnected: boolean }) =>
      s.authType !== "oauth" || s.isOAuthConnected;
    const enabledCount = servers.filter(
      (s) => s.isEnabled && isReady(s),
    ).length;
    ...
    const handleToggle = (id: string, currentEnabled: boolean) => {
      ...
      queryClient.setQueryData(["mcpServers", user?.id], (old: any) => {
        ...
        return {
          ...old,
          servers: old.servers.map((s: any) =>
            s.id === id ? { ...s, isEnabled: !currentEnabled } : s,
          ),
        };
      });
    ...
    const serverItems = (
      <div className="flex flex-col min-h-0">
        ...
        <div
          className={cn(
            "overflow-y-auto max-h-[70vh] sm:max-h-[260px] px-1.5 pb-1.5",
            !showSearch && "pt-1.5",
          )}
        >
          {filteredServers.length === 0 ? (
            ...
            filteredServers.map((server) => {
              ...
              const isGitHub = MCP_COMPONENT_ICON_URLS.has(
                server.url.replace(/\/+$/, ""),
              );
                ...
                <div
                  ...
                  className={cn(
                    ...
                    ready
                      ...
                      : "hover:bg-accent/50 cursor-pointer opacity-80",
                  )}
                >
                  {isGitHub ? (
                    <GithubLogoIcon
                  ...
                  <Checkbox
                    ...
                    onCheckedChange={() =>
                      ready && handleToggle(server.id, server.isEnabled)
                    }
    ...
    const drawerItems = (
      <div className="flex flex-col min-h-0 flex-1">
        ...
        <div
          className={cn(
            "overflow-y-auto flex-1 px-2 pb-2",
            !showSearch && "pt-2",
          )}
        >
          {filteredServers.length === 0 ? (
            ...
            filteredServers.map((server) => {
              ...
              const isGitHubMobile = MCP_COMPONENT_ICON_URLS.has(
                server.url.replace(/\/+$/, ""),
              );
        ...
        <Drawer open={open} onOpenChange={handleOpenChange}>
          ...
          <DrawerContent className="max-h-[80vh] h-full flex flex-col overflow-hidden">
            <DrawerHeader className="text-left pb-1 shrink-0">
              <DrawerTitle className="text-lg font-light tracking-tight font-be-vietnam-pro">
                scira apps
              </DrawerTitle>
            </DrawerHeader>
...
const WEB_SEARCH_PROVIDERS: Array<{
...
}> = [
  {
    ...
    label: "Exa",
    description:
      "Enhanced and faster neural web search with images and filtering.",
  },
  {
    value: "firecrawl",
    label: "Firecrawl",
  ...
  },
  {
    ...
    label: "Parallel AI",
    description:
      "Base and premium web search with Parallel’s Firecrawl image support.",
  },
];
...
const GroupModeToggle: React.FC<GroupSelectorProps> = React.memo(
  ({
    ...
    const messageCountExceeded = Boolean(
      !isProUser &&
      usageData &&
      usageData.messageCount >= SEARCH_LIMITS.DAILY_SEARCH_LIMIT,
    );
    ...
    const [searchProvider, setSearchProvider] = useLocalStorage<SearchProvider>(
    ...
    const visibleGroups = useMemo(
      () =>
        dynamicSearchGroups.filter((group) => {
          ...
          if ("requireAuth" in group && group.requireAuth && !isAuthenticated)
            return false;
    ...
    const [visibleModes] = useSyncedPreferences<string[]>(
      "scira-visible-modes",
      [],
    );
    const [modeOrderInner] = useSyncedPreferences<string[]>(
      "scira-group-order",
      [],
    );
    ...
    const extremeTooltipContent = useMemo(() => {
      ...
      if (!isProUser && extremeSearchCountExceeded && !isExtreme) {
        return (
          <div className="grid gap-2 text-left">
            ...
            <p className="text-xs leading-snug text-muted-foreground">
              You've used {SEARCH_LIMITS.EXTREME_SEARCH_LIMIT} extreme searches
              this month.
            </p>
            <div className="grid gap-1.5 border-t border-border pt-2">
              <div className="flex items-center gap-2 text-xs font-semibold text-foreground">
      ...
      return (
        <div className="grid gap-2 text-left">
          ...
          <p className="text-xs leading-snug text-muted-foreground">
            Extreme research with multiple sources and in-depth analysis with 3x
            sources.
          </p>
          {!isProUser && usageData && (
            <p className="text-[11px] text-muted-foreground/80">
              {usageData.extremeSearchCount} /{" "}
              {SEARCH_LIMITS.EXTREME_SEARCH_LIMIT} used this month
            </p>
          )}
          {!isProUser && (
            <div className="grid gap-1.5 border-t border-border pt-2">
    ...
    const handleWebProviderChange = useCallback(
      (provider: SearchProvider) => {
        ...
        const label =
          WEB_SEARCH_PROVIDERS.find((option) => option.value === provider)
            ?.label ?? provider;
        sileo.success({
      ...
      async (currentValue: string) => {
        ...
        if (selectedGroup) {
          ...
          if (
            ...
            try {
              const { listUserConnectorsAction } =
            ...
            return (
              <Tooltip key={group.id}>
                ...
                <TooltipContent side="bottom" className="text-xs max-w-[160px]">
                  ...
                  <p className="text-primary-foreground/80 text-[10px] leading-snug">
                    {group.description}
                  </p>
                </TooltipContent>
              </Tooltip>
                ...
                <TooltipTrigger>
                  <DrawerTrigger>
                    <Button
                      ...
                      disabled={
                        !isProUser && !isExtreme && messageCountExceeded
                      }
                      className={cn(
          ...
          <Tooltip>
            <TooltipTrigger>
              <Button
                ...
                disabled={
                  !isProUser && extremeSearchCountExceeded && !isExtreme
                }
                className={cn(
...
const FormComponent: React.FC<FormComponentProps> = ({
  ...
  const [searchProvider] = useSyncedPreferences<SearchProvider>(
    "scira-search-provider",
    "exa",
  );
  const dynamicSearchGroups = useMemo(
  ...
  const [visibleModesOuter] = useSyncedPreferences<string[]>(
    "scira-visible-modes",
    [],
  );
  const [modeOrderOuter] = useSyncedPreferences<string[]>(
    "scira-group-order",
    [],
  );
  const isExtreme = selectedGroup === "extreme";
  ...
  const plusMenuGroups = useMemo(() => {
    let filtered = dynamicSearchGroups.filter((group) => {
      ...
      if (group.id === "extreme" || group.id === "canvas" || group.id === "mcp")
        return false;
      return true;
    });
  ...
  const filteredTriggerGroups = useMemo(() => {
    ...
    return triggerBaseGroups.filter(
      (g) =>
  ...
  const showMultiAgentInTrigger = useMemo(() => {
    ...
    const isAlreadyInModeGroups = filteredTriggerGroups.some(
      (group) => group.id === "multi-agent",
    );
    if (isAlreadyInModeGroups) return false;
    if (!triggerFilter) return true;
    const lower = triggerFilter.toLowerCase();
    return (
      "multi-agent".includes(lower) ||
      "multi agent".includes(lower) ||
      "research".includes(lower)
    );
  }, [
    triggerPopup,
    user,
  ...
  const triggerTotalItems =
    filteredTriggerGroups.length +
    (showMultiAgentInTrigger ? 1 : 0) +
    (showTempInTrigger ? 1 : 0);
  ...
  useEffect(() => {
    ...
    const item = container.querySelector(
      `[data-at-index="${triggerHighlightIndex}"]`,
    );
    if (item) {
    ...
    return () => {
      ...
      if (pendingTypewriterTargetRef.current) {
        ...
        const typedLength = Math.min(
          target.length,
          Math.max(latestInputRef.current.length, 1),
        );
  ...
  useEffect(() => {
    ...
    const resumeIndex = Math.min(
      pendingResume.target.length,
      Math.max(pendingResume.index, currentInput.length, 1),
    );
    ...
    if (isRecording && mediaRecorderRef.current) {
      ...
      try {
        ...
        if (typeof window === "undefined") {
          haptics.trigger("error");
          sileo.error({
            title: "Voice recording is only available in the browser.",
          });
          return;
        }
        ...
        recorder.addEventListener("dataavailable", async (event) => {
          if (event.data.size > 0) {
            ...
            try {
              ...
              haptics.trigger("error");
              sileo.error({
                title: "Failed to transcribe audio. Please try again.",
              });
            } finally {
              cleanupMediaRecorder();
            }
        ...
        recorder.addEventListener("error", (e) => {
          ...
          haptics.trigger("error");
          sileo.error({
            title: "Recording failed. Please try again or switch browser.",
          });
          cleanupMediaRecorder();
        });
        ...
        haptics.trigger("error");
        sileo.error({
          title: "Could not access microphone. Please allow mic permission.",
        });
        setIsRecording(false);
      }
    }
  ...
  const suggestDebouncer = useDebouncer(fetchSuggestions, {
    wait: 50,
    key: "autocomplete",
  });
  ...
  const handleInput = useCallback(
    (event: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      if (newValue.length > MAX_INPUT_CHARS) {
        setInput(newValue);
        sileo.error({
          title: `Your input exceeds the maximum of ${MAX_INPUT_CHARS} characters.`,
        });
      } else {
        setInput(newValue);
      }
      ...
      if (!isEnhancing && !isTypewriting) {
        ...
        if (group.id === "extreme" && !supportsExtremeMode(selectedModel)) {
          const extremeModels = models.filter((model) => {
            ...
            if (requiresProSubscription(model.value) && !isProUser)
              return false;
            return true;
          });

          if (extremeModels.length > 0) {
            const defaultModel = extremeModels.find(
              (m) => m.value === "scira-default",
            );
            const firstFreeModel = extremeModels.find((m) => !m.pro);
            const targetModel =
              defaultModel || firstFreeModel || extremeModels[0];
        ...
        if (group.id === "canvas" && !supportsCanvasMode(selectedModel)) {
          const canvasModels = models.filter((m) =>
            supportsCanvasMode(m.value),
          );
          const preferred =
            canvasModels.find((m) => m.value === "scira-code") ||
            canvasModels[0];
          if (preferred) {
  ...
  const handleTriggerMultiAgentSelect = useCallback(() => {
    ...
    const isTurningOff =
      selectedGroup === "multi-agent" || isMultiAgentModeEnabled;
  ...
  const uploadFile = useCallback(async (file: File): Promise<Attachment> => {
    try {
      ...
      if (!presignResponse.ok) {
        ...
        throw new Error(
          `Failed to get upload URL: ${presignResponse.status} ${errorText}`,
        );
      }
  ...
  const removeAttachment = useCallback(
    async (index: number) => {
      ...
      if (
        ...
        attachmentToDelete.url.includes("/scira/")
      ) {
        try {
  ...
  const handleDrop = useCallback(
    async (e: React.DragEvent) => {
      ...
      if (unsupportedFiles.length > 0) {
        ...
        sileo.error({
          title: `Some files not supported: ${unsupportedFiles.map((f) => f.name).join(", ")}`,
        });
      }
      ...
      if (oversizedFiles.length > 0) {
        ...
        sileo.error({
          title: `Some files exceed the 5MB limit: ${oversizedFiles.map((f) => f.name).join(", ")}`,
        });
      }
      ...
      if (
        ...
        sileo.error({
          ...
          description:
            "Only image, PDF, and document files (CSV, DOCX, XLSX) are supported",
          icon: <FileText className="h-4 w-4" />,
        });
        return;
      }
      ...
      setTimeout(async () => {
        try {
          ...
          sileo.error({
            title: "Upload failed. Please check console for details.",
          });
        } finally {
          setUploadQueue([]);
        }
  ...
  const handlePaste = useCallback(
    async (e: React.ClipboardEvent) => {
      ...
      const oversizedFiles = files.filter(
        (file) => file.size > getMaxSizeForFile(file),
      );
      ...
      const filesToUpload =
        oversizedFiles.length > 0
          ? files.filter((file) => file.size <= getMaxSizeForFile(file))
          : files;
      ...
      try {
        ...
        const uploadedAttachments = filesToUpload.map(
          (_, index) => uploadMap[`file:${index}`],
        );
        ...
        sileo.error({
          title: "Failed to upload pasted image. Please try again.",
        });
      } finally {
        setUploadQueue([]);
      }
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (triggerPopup && triggerTotalItems > 0) {
        ...
        if (event.key === "ArrowUp") {
          event.preventDefault();
          setTriggerHighlightIndex(
            (prev) => (prev - 1 + triggerTotalItems) % triggerTotalItems,
          );
          return;
        }
        if (event.key === "Enter" && !isCompositionActive.current) {
          ...
          if (triggerHighlightIndex < filteredTriggerGroups.length) {
            ...
            triggerHighlightIndex === filteredTriggerGroups.length
          ) {
            handleTriggerMultiAgentSelect();
          } else if (
            showTempInTrigger &&
            triggerHighlightIndex ===
              filteredTriggerGroups.length + (showMultiAgentInTrigger ? 1 : 0)
          ) {
          ...
          <div className="relative" data-form-container>
            {triggerPopup && triggerTotalItems > 0 && (
              ...
              >
                ...
                <div
                  ref={triggerPopupScrollRef}
                  className="px-1 pb-1 max-h-[260px] overflow-y-auto scrollbar-thin"
                >
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                      ...
                      <DrawerTrigger>
                        <button
                          className={cn(
                            ...
                            plusMenuOpen &&
                              "bg-foreground/18 text-foreground border-foreground/35",
                          )}
                        ...
                        <div className="px-1 pb-4 max-h-[calc(70vh-80px)] overflow-y-auto">
                          ...
                          {user && mcpEnabled && (
                            <button
                              ...
                              onClick={async () => {
                                const g = dynamicSearchGroups.find(
                                  (g) => g.id === "mcp",
                                );
                          ...
                          {user && !hasInteracted && (
                            ...
                            >
                              ...
                              <span className="text-[9px] text-muted-foreground ml-auto hidden sm:block">
                                ⌘⇧J
                              </span>
                            </button>
                          )}
                      ...
                      >
                        <Tooltip>
                          <TooltipTrigger>
                            ...
                            >
                              ...
                              <Kbd className="text-[9px] h-4 min-w-4 px-1">
                                ⌘U
                              </Kbd>
                            </button>
                          </TooltipTrigger>
                          <TooltipContent
                            side="right"
                            sideOffset={8}
                            className="max-w-48 py-1.5 px-2.5"
                          >
                        ...
                        {user && setIsMultiAgentModeEnabled && (
                          <Tooltip>
                            ...
                            >
                              <span className="text-[10px] leading-snug">
                                Use xAI multi-agent mode with web and X tool
                                calls plus sources
                              </span>
                            </TooltipContent>
                          </Tooltip>
                        ...
                        {user && mcpEnabled && (
                          <Tooltip>
                            ...
                            >
                              <span className="text-[10px] leading-snug">
                                Use tools from your connected apps
                              </span>
                            </TooltipContent>
                          </Tooltip>
                        )}
                        ...
                        <div className="relative">
                          ...
                          >
                            {plusMenuGroups.map((group) => {
                              const sel =
                                selectedGroup === group.id && !isExtreme;
                              return (
                                <Tooltip key={group.id}>
                                  <TooltipTrigger>
                                    <button
                                      ...
                                      onClick={() =>
                                        handlePlusMenuGroupSelect(group)
                                      }
                                    >
                                      <FlexibleIcon
                                        icon={group.icon}
                  ...
                  {/* Active mode badge */}
                  {(() => {
                    const activeGroup = dynamicSearchGroups.find(
                  ...
                  {selectedGroup === "mcp" && (
                    <McpServerSelector user={user} isProUser={isProUser} />
                  )}
                </div>
                ...
                uploadQueue.length === 0 && (
                  ...
                  >
                    ...
                    {suggestions.map((suggestion, index) => {
                      ...
                      const matchEnd = lower.startsWith(query)
                        ? query.length
                        : 0;

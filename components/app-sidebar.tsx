...
export const AppSidebar = memo(
  ({ user, onHistoryClick, isProUser }: AppSidebarProps) => {
    const [blurPersonalInfo] = useSyncedPreferences<boolean>(
    ...
    const [shareVisibility, setShareVisibility] =
      React.useState<VisibilityType>("private");
    ...
    const pinnedRecentChats = useMemo(
      () => recentChats.filter((chat) => chat.isPinned),
      [recentChats],
    );

    const unpinnedRecentChats = useMemo(
      () => recentChats.filter((chat) => !chat.isPinned),
      [recentChats],
    );
    ...
    const groupedChats = useMemo(
      () => groupChatsByDate(unpinnedRecentChats),
      [unpinnedRecentChats],
    );
    ...
    const togglePinnedChat = async (chatId: string) => {
      ...
      try {
        const updatedChat = await updateChatPinned(
          chatId,
        ...
        queryClient.setQueryData(["recent-chats", user?.id], (oldData: any) => {
          ...
          return {
            ...
            chats: oldData.chats.map((chat: any) =>
              chat.id === chatId
                ? { ...chat, isPinned: !selectedChat.isPinned }
                : chat,
            ),
          };
        });
    ...
    const handleShareVisibilityChange = async (visibility: VisibilityType) => {
      ...
      try {
        ...
        const shareUrl =
          visibility === "public"
            ? `https://scira.ai/share/${shareTarget.id}`
            : "";
        sileo.success({
          title:
            visibility === "public" ? "Chat shared" : "Chat is now private",
          description:
            visibility === "public"
              ...
              <div className="relative flex items-center w-full h-12 px-2 overflow-visible">
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex flex-row items-center gap-2 leading-none group-data-[collapsible=icon]:hidden">
                      <span className="font-be-vietnam-pro font-light tracking-tighter text-xl">
                        scira
                      </span>
                      {user && isProUser && (
                        <div className="w-fit">
                ...
                <div className="absolute top-2 right-2 group-data-[collapsible=icon]:hidden">
                  <Tooltip>
                    ...
                    >
                      Close Sidebar{" "}
                      <span className="text-xs text-secondary pl-0.5">⌘B</span>
                    </TooltipContent>
                  </Tooltip>
                </div>
          ...
          <SidebarMenu className="group-data-[collapsible=icon]:items-center group-data-[collapsible=icon]:justify-center">
            ...
            {user && (
              <div className="px-2 py-1.5 group-data-[collapsible=icon]:hidden">
                <span className="font-pixel text-[11px] text-muted-foreground/60 uppercase tracking-[0.12em]">
                  Tools
                </span>
              </div>
            )}
                      ...
                      {pinnedRecentChats.length > 0 && (
                        <div className="mb-2">
                          ...
                          {pinnedRecentChats.map((chat: any) => {
                            ...
                            const normalizedVisibility: VisibilityType =

  ...
  const startCapture = useCallback(
    async (onChunk: (base64Audio: string) => void): Promise<number> => {
      ...
      stream.getTracks().forEach((track) => {
        track.onended = () => {
          setError(
            "Microphone disconnected. Please check your device and try again.",
          );
          cleanupRef.current();
        };
      });
        ...
        const pushToolOutput = (tool: {
          ...
          if (callId) {
            ...
            if (startedAt) {
              ...
              setStats((prev) => ({
                ...prev,
                lastToolLatencyMs: toolLatencyMs,
              }));
            }
          }
        ...
        switch (type) {
          case "conversation.created": {
            if (!isSessionConfiguredRef.current && sessionConfigRef.current) {
              const { voice: backendVoice, instructions: backendInstructions } =
                sessionConfigRef.current;
          ...
          case "response.created": {
            currentResponseIdRef.current =
              (message.response as { id?: string } | undefined)?.id ?? null;

...
export async function generateTitleFromUserMessage({
  ...
  const prompt = JSON.stringify(
    firstTextPart && firstTextPart.type === "text" ? firstTextPart.text : "",
  );
  ...
  const { text: title } = await generateText({
    ...
    onFinish: (output) => {
      ...
      console.log(
        `⏱️ [USAGE] generateTitleFromUserMessage: Model took ${durationMs}ms`,
      );
    },
  });
...
export async function getGroupConfig(
  ...args: Parameters<typeof getSearchGroupConfig>
) {
  "use server";
...
export async function bulkDeleteChats(chatIds: string[]) {
  ...
  try {
    const taskEntries = chatIds.map(
      (id) => [`chat:${id}`, async () => deleteChatById({ id })] as const,
    );
    ...
    const deletedCount = settledValues.filter(
...
export async function forkChat(
  ...
  try {
    ...
    const newChatTitle = originalChat.title
      ? `Fork of ${originalChat.title}`
      : "Forked Chat";
...
export async function branchOutChat({
  ...
  try {
    ...
    const chatTitlePromise = generateTitleFromUserMessage({
      message: userMessage,
    });
    ...
    const messagesToSave = [
      ...
      },
      {
        ...
        completionTime:
          (assistantMessage as any).metadata?.completionTime ?? null,
      },
    ];
...
export async function previewMaxUpgrade() {
  ...
  try {
    ...
    if (!maxProductId) {
      return {
        success: false,
        error: "NEXT_PUBLIC_MAX_TIER environment variable is required",
      };
    }
    ...
    if (userData.proSource !== "dodo") {
      return {
        success: false,
        error: "Preview is only available for active Dodo subscriptions",
      };
    }
    ...
    if (!dodoProProductId) {
      return {
        success: false,
        error: "NEXT_PUBLIC_PREMIUM_TIER environment variable is required",
      };
    }
    ...
    if (!activeDodoProSub?.id) {
      return {
        success: false,
        error: "Active Dodo Pro subscription not found",
      };
    }
    ...
    return {
      ...
      preview: {
        ...
        settlementAmount: preview.immediate_charge.summary.settlement_amount,
        settlementCurrency:
          preview.immediate_charge.summary.settlement_currency,
        lineItems: preview.immediate_charge.line_items,
      },
    };
  } catch (error) {
    ...
    return {
      success: false,
      error: "Failed to preview Max upgrade. Please try again.",
    };
  }
}
...
export async function previewDowngradeToPro() {
  ...
  try {
    ...
    if (!userData.isMaxUser || userData.proSource !== "dodo") {
      return {
        success: false,
        error: "Preview is only available for active Dodo Max subscriptions",
      };
    }
    ...
    const activeDodoMaxSub = await maindb.query.dodosubscription.findFirst({
      where: and(
        eq(dodosubscription.userId, user.id),
        eq(dodosubscription.productId, dodoMaxProductId),
      ),
    ...
    if (!activeDodoMaxSub?.id) {
      return {
        success: false,
        error: "Active Dodo Max subscription not found",
      };
    }
    ...
    return {
      success: false,
      error: "Failed to preview Pro downgrade. Please try again.",
    };
  }
}
...
export async function downgradeToPro() {
  ...
  try {
    ...
    if (!userData.isMaxUser || userData.proSource !== "dodo") {
      return {
        success: false,
        error: "Downgrade is only available for active Dodo Max subscriptions",
      };
    }
    ...
    return {
      success: false,
      error: "Failed to downgrade to Pro. Please try again.",
    };
  }
}
...
export async function getUserMessageCount(providedUser?: User | null) {
  ...
  try {
    ...
    console.log(
      `⏱️ [USAGE] getUserMessageCount: DB usage lookup took ${durationMs}ms`,
    );
...
export async function getUserExtremeSearchCount(providedUser?: User | null) {
  ...
  try {
    ...
    const durationMs = Date.now() - start;
    console.log(
      `⏱️ [USAGE] getUserExtremeSearchCount: DB usage lookup took ${durationMs}ms`,
    );
...
export async function getExtremeSearchUsageCount(providedUser?: User | null) {
  ...
  try {
    ...
    const durationMs = Date.now() - start;
    console.log(
      `⏱️ [USAGE] getExtremeSearchUsageCount: DB usage lookup took ${durationMs}ms`,
    );
...
export async function getAnthropicUsageCountAction(providedUser?: User | null) {
  ...
  try {
    ...
    const durationMs = Date.now() - start;
    console.log(
      `⏱️ [USAGE] getAnthropicUsageCountAction: DB usage lookup took ${durationMs}ms`,
    );
...
export async function getAgentModeUsageCountAction(providedUser?: User | null) {
  ...
  try {
    ...
    const durationMs = Date.now() - start;
    console.log(
      `⏱️ [USAGE] getAgentModeUsageCountAction: DB usage lookup took ${durationMs}ms`,
    );
...
export async function incrementAnthropicUsageAction(model?: string | null) {
  ...
  try {
    ...
    return {
      success: false,
      error: "Failed to increment anthropic usage count",
    };
  }
}
...
export async function getGoogleUsageCountAction(providedUser?: User | null) {
  ...
  try {
    ...
    const durationMs = Date.now() - start;
    console.log(
      `⏱️ [USAGE] getGoogleUsageCountAction: DB usage lookup took ${durationMs}ms`,
    );
 ...
 */
export async function getMessageCountAndExtremeSearchByUserIdAction(
  userId: string,
): Promise<{
  messageCountResult:
    | { count: number; error: null }
    | { count: undefined; error: Error };
  extremeSearchUsage:
    | { count: number; error: null }
    | { count: undefined; error: Error };
  ...
  try {
    const { messageCount, extremeSearchCount, anthropicCount } =
      await getMessageCountAndExtremeSearchByUserId({
        userId,
      });

    if (messageCached === null)
      usageCountCache.set(messageCacheKey, messageCount);
    if (extremeCached === null)
      usageCountCache.set(extremeCacheKey, extremeSearchCount);
    if (anthropicCached === null)
      usageCountCache.set(anthropicCacheKey, anthropicCount);
    ...
    const error =
      err instanceof Error ? err : new Error("Failed to verify usage limits");
    return {
  ...
  try {
    ...
    if (isIndianUser === undefined) {
      try {
        ...
        console.warn(
          "Geolocation lookup failed in getDiscountConfigAction:",
          geoError,
        );
        isIndianUser = false;
      }
...
`;
    ...
    const matchedRoute = sanitizedRoutes.find(
      (route) => route.name === routeName,
    );
    let resolvedModel = matchedRoute?.model || "scira-default";

    if (hasImages && !hasVisionSupport(resolvedModel)) {
      const visionRoute = sanitizedRoutes.find((route) =>
        hasVisionSupport(route.model),
      );
...
export async function getDodoSubscriptionHistory() {
  try {
    ...
    const subscriptions = await getDodoSubscriptionsByUserId({
      userId: user.id,
    });
...
export async function getAllChatsWithPreview(
  limit: number = 25,
  offset: number = 0,
) {
  "use server";
      ...
      where: and(
        ...
        notExists(
          db
            ...
            .from(buildSession)
            .where(eq(buildSession.chatId, chat.id)),
        ),
      ),

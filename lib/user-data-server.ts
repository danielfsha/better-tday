...
function getActiveDodoSubscriptions(
...
function getDodoExpirationInfoFromSubscriptions(
  ...
  const activeWithEndDate = getActiveDodoSubscriptions(
    subscriptions,
    now,
...
const userDataCache = new LruTtlCache<ComprehensiveUserData>(
  USER_DATA_CACHE_MAX,
);
const lightweightAuthCache = new LruTtlCache<LightweightUserAuth>(
  LIGHTWEIGHT_AUTH_CACHE_MAX,
);

// Custom instructions cache (per-user)
const customInstructionsCache = new LruTtlCache<CustomInstructions | null>(
  CUSTOM_INSTRUCTIONS_CACHE_MAX,
);
const CUSTOM_INSTRUCTIONS_CACHE_TTL_MS = 15 * 60 * 1000; // 15 minutes

// User preferences cache (per-user)
const userPreferencesCache = new LruTtlCache<UserPreferences | null>(
  USER_PREFERENCES_CACHE_MAX,
);
 ...
 */
export const getLightweightUserAuth = cache(
  async (): Promise<LightweightUserAuth | null> => {
    try {
      ...
      const DODO_MAX_PRODUCT_ID =
        process.env.DODO_MAX_PRODUCT_ID || process.env.NEXT_PUBLIC_MAX_TIER;
      ...
      }>(
        {
          ...
          async dodoCheck() {
            if (cachedDodoStatus !== null) {
              const isDodoActive =
                cachedDodoStatus.isProUser ??
                cachedDodoStatus.hasSubscriptions ??
                false;
              if (isDodoActive) {
            ...
            if (recentDodoSubscription.length > 0) {
              ...
              isMaxUser =
                isDodoActive &&
                !!DODO_MAX_PRODUCT_ID &&
                sub.productId === DODO_MAX_PRODUCT_ID;
            }
            setDodoProStatus(userId, {
              isProUser: isDodoActive,
              hasSubscriptions: isDodoActive,
              isMaxUser,
            });
            ...
            if (isDodoActive) {
              const rows = await this.$.polarQuery;
              if (rows?.length)
                this.$end({ email: rows[0].email, isProUser: true, isMaxUser });
            }
            return isDodoActive;
          },
        },
...
export const getComprehensiveUserData = cache(
  async (): Promise<ComprehensiveUserData | null> => {
    try {
      // Get session once
      const session = await auth.api.getSession({
      ...
      const now = new Date();
      const activeDodoSubscriptions = getActiveDodoSubscriptions(
        dodoSubscriptions,
        now,
      );
      const dodoExpirationInfo = getDodoExpirationInfoFromSubscriptions(
        dodoSubscriptions,
        now,
      );
      ...
      if (hasDodoSubscriptions) {
        ...
        if (mostRecentSubscription.currentPeriodEnd) {
          isDodoActive =
            new Date(mostRecentSubscription.currentPeriodEnd) > now;
        } else {
        ...
        isDodoMax =
          isDodoActive &&
          !!DODO_MAX_PRODUCT_ID &&
          mostRecentSubscription.productId === DODO_MAX_PRODUCT_ID;
      }
      ...
      let subscriptionStatus: "active" | "canceled" | "expired" | "none" =
        "none";
      ...
      if (dodoSubscriptions.length > 0 || proSource === "dodo") {
        comprehensiveData.dodoSubscription = {
          ...
          mostRecentSubscription: hasDodoSubscriptions
            ? activeDodoSubscriptions[0].createdAt
            : undefined,
